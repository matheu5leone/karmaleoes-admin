...
Validar cadastro manual, lifecycle (Em aberto / Encerrado), status dinâmicos vinculados a lifecycle, campo Enable para visibilidade, expiração automática (comportamento distinto por lifecycle), encerramento manual com restrições de data, gestão de status e regras de exibição no Hub (RN-EVENTO-001 a 015).
...
|----|------------|------|---------|--------|-------------------|
| TC-EVENTO-001 | P0 | E2E | Fluxo principal completo | Cadastrar (enable false) → habilitar (enable true) → visualizar no Hub → link compra → expirar (enable false, Expirado) → encerrar Sucesso | Ciclo completo conforme especificação |
...
| TC-EVENTO-024 | P0 | Negativo | Edição de "Expirado" | Tentar editar o status "Expirado" | Operação bloqueada (RN-EVENTO-009) |
| TC-EVENTO-024b | P0 | Negativo | Edição de "Adiado" | Tentar editar o status "Adiado" | Operação bloqueada (RN-EVENTO-009) |
| TC-EVENTO-025 | P0 | Negativo | Exclusão de "Expirado" | Tentar excluir o status "Expirado" | Operação bloqueada (RN-EVENTO-009) |
...
| ID | Prioridade | Tipo | Cenário | Passos | Resultado esperado |
|----|------------|------|---------|--------|-------------------|
| TC-EVENTO-006 | P0 | Positivo | Expiração com lifecycle Em aberto | Evento Em aberto com data de referência de ontem | Status "Expirado"; lifecycle "Em aberto"; enable `false` (RN-EVENTO-005) |
| TC-EVENTO-006b | P0 | E2E | Expiração desabilita Hub | Evento expirado automaticamente | Evento não visível no Hub (enable false) |
| TC-EVENTO-006c | P0 | Negativo | Evento no dia, ainda visível | Evento com data de referência de hoje e enable true | Evento ainda visível; expiração ocorre no dia seguinte |
| TC-EVENTO-006d | P0 | Positivo | Expiração usa nova data após Adiado | Evento Adiado com nova data de ontem | Expiração baseada em nova data, não na data original (RN-EVENTO-008) |
| TC-EVENTO-006e | P0 | Positivo | Expiração de evento Encerrado/Cancelado | Evento Encerrado/Cancelado com enable true antes da expiração | Enable `false`; lifecycle "Encerrado"; status "Cancelado" inalterados (RN-EVENTO-005/015) |
| TC-EVENTO-006f | P0 | Positivo | Expiração de evento Encerrado/Sucesso | Evento Encerrado/Sucesso com enable true | Enable `false`; lifecycle "Encerrado"; status "Sucesso" inalterados (RN-EVENTO-005) |
...
| TC-EVENTO-007f | P0 | Positivo | Encerramento de evento Expirado | Encerrar evento com status "Expirado" como "Sucesso" | Lifecycle "Encerrado"; status "Sucesso"; enable inalterado |
...
| TC-EVENTO-043 | P1 | E2E | Encerrado + enable true = visível | Evento Encerrado/Cancelado com enable true antes da expiração | Evento visível no Hub até expiração automática |
...
| TC-EVENTO-042 | RN-EVENTO-015 | P0 | E2E | Cancelado: expiração seta enable false | Encerrar Cancelado (enable true) → aguardar expiração | Enable `false`; lifecycle Encerrado; status Cancelado mantidos |
...
- [ ] Proteção dos status "Expirado" e "Adiado" (P0)
- [ ] Exclusão de status em uso bloqueada (P0)
- [ ] Adiado exige nova data; expiração usa nova data (P0)
- [ ] Expiração Em aberto: status Expirado + enable false (P0)
