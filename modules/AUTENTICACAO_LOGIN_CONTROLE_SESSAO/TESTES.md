...
Validar autenticação administrativa (CPF, senha, 2FA), configuração inicial do 2FA, recuperação de senha, sessão única com expiração por inatividade, gestão de usuários (com telefone obrigatório), bootstrap e auditoria conforme RF-LOGIN-001 a 007 e RN-LOGIN-001 a 007.
...
|----|------------|------|---------|---------------|--------|-------------------|
| TC-LOGIN-001 | P0 | Positivo | Login com credenciais válidas | Usuário cadastrado e ativo | Informar CPF e senha corretos; prosseguir para 2FA | Sistema aceita credenciais e solicita segundo fator |
| TC-LOGIN-002 | P0 | Negativo | CPF inexistente | CPF não cadastrado | Informar CPF e senha | Acesso negado; mensagem adequada |
| TC-LOGIN-003 | P0 | Negativo | Senha incorreta | Usuário cadastrado | Informar CPF válido e senha inválida | Acesso negado; sem revelar qual campo falhou (se política de segurança) |
| TC-LOGIN-004 | P1 | Negativo | Campos obrigatórios vazios | Tela de login | Submeter formulário sem CPF ou senha | Validação impede envio |

## RF-LOGIN-002 — Autenticação em 2 Fatores

| ID | Prioridade | Tipo | Cenário | Pré-condições | Passos | Resultado esperado |
|----|------------|------|---------|---------------|--------|-------------------|
| TC-LOGIN-005 | P0 | Positivo | 2FA com código TOTP válido | CPF/senha validados; app configurado | Informar código TOTP correto | Sessão autenticada; acesso ao admin |
| TC-LOGIN-006 | P0 | Negativo | Código TOTP inválido | CPF/senha validados | Informar código incorreto ou expirado | Acesso negado |
...
| ID | Prioridade | Tipo | Cenário | Pré-condições | Passos | Resultado esperado |
|----|------------|------|---------|---------------|--------|-------------------|
| TC-LOGIN-008 | P0 | Positivo | Fluxo completo de recuperação | Usuário com telefone cadastrado | 1) Informar CPF 2) Receber SMS 3) Validar código 4) Redefinir senha | Senha alterada; login possível com nova senha |
| TC-LOGIN-009 | P1 | Negativo | Código SMS inválido | Código enviado | Informar código errado | Redefinição bloqueada |
| TC-LOGIN-010 | P1 | Negativo | CPF sem telefone cadastrado | Usuário sem número | Solicitar recuperação | Fluxo interrompido com orientação adequada |
...
| ID | Prioridade | Tipo | Cenário | Pré-condições | Passos | Resultado esperado |
|----|------------|------|---------|---------------|--------|-------------------|
| TC-LOGIN-013 | P0 | Positivo | Cadastro de usuário | Admin autenticado | Cadastrar com CPF, telefone e senha temporária | Usuário criado e apto a autenticar |
...
| TC-LOGIN-013c | P0 | Negativo | CPF duplicado | CPF já cadastrado | Tentar cadastrar com mesmo CPF | Operação bloqueada (RN-LOGIN-007) |
| TC-LOGIN-014 | P0 | Positivo | Edição de telefone | Usuário existente | Alterar telefone | Alteração persistida |
| TC-LOGIN-014b | P1 | Negativo | Edição de CPF | Usuário existente | Tentar alterar CPF | Operação bloqueada; CPF imutável |
...
| ID | Prioridade | Tipo | Cenário | Pré-condições | Passos | Resultado esperado |
|----|------------|------|---------|---------------|--------|-------------------|
| TC-LOGIN-022 | P0 | Positivo | Primeiro login com senha temporária | Usuário recém-cadastrado (CPF + telefone + senha temp.) | 1) Login com CPF + senha temporária 2) Sistema exibe QR code 3) Configurar app 4) Validar primeiro código | 2FA configurado; logins seguintes exigem TOTP |
...
- [ ] Cadastro de usuário com CPF + telefone + senha temporária (P0)
